...
public static class RepeatExtension
{
    ...
    /// <param name="pipeline">The pipeline to execute.</param>
    /// <param name="count">The number of times to execute the pipeline. Must be greater than 0.</param>
    /// <returns>An enumerable containing the results of each execution.</returns>
    /// <exception cref="ArgumentOutOfRangeException">Thrown if <paramref name="count"/> is less than 1.</exception>
    [DebuggerStepperBoundary]
    public static IEnumerable<Result<T, TError>> Repeat<T, TError>(this IPipeline<T, TError> pipeline, int count)
    {
        if (count < 1)
        {
            throw new ArgumentOutOfRangeException(nameof(count), "Count must be greater than 0.");
    ...
    /// <summary>
    /// Async version of Repeat with a discriminated error type.
    /// </summary>
    /// <typeparam name="T">The value type.</typeparam>
    /// <typeparam name="TError">The error type.</typeparam>
    /// <param name="pipeline">The asynchronous pipeline to execute.</param>
    /// <param name="count">The number of times to execute the pipeline. Must be greater than 0.</param>
    /// <returns>An async enumerable containing the results of each execution.</returns>
    /// <exception cref="ArgumentOutOfRangeException">Thrown if <paramref name="count"/> is less than 1.</exception>
    [DebuggerStepperBoundary]
    public static async IAsyncEnumerable<Result<T, TError>> RepeatAsync<T, TError>(this IAsyncPipeline<T, TError> pipeline, int count)
    {
        if (count < 1)
        {
            throw new ArgumentOutOfRangeException(nameof(count), "Count must be greater than 0.");
    ...
    /// <exception cref="ArgumentOutOfRangeException">Thrown if <paramref name="count"/> is less than 1.</exception>
    [DebuggerStepperBoundary]
    public static async Task<IEnumerable<Result<T, TError>>> RepeatAsync<T, TError>(this IAsyncPipeline<T, TError> pipeline, int count)
    {
        if (count < 1)
        {
            throw new ArgumentOutOfRangeException(nameof(count), "Count must be greater than 0.");
        ...
        var results = new List<Result<T, TError>>();
